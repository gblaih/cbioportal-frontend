...
export default class Mutations extends React.Component<IMutationProps, {}> {
    ...
    constructor(props: IMutationProps) {
        ...
        this.mutationMapperStore = new MutationMapperToolStore(
            this.props.store.mutations.result
            // {groupFilters:
            //     this.props.store.groupedSamples.map(group =>
            //         ({
            //             group: group.name,
            //             filter: {type: GROUP_COMPARISON_FILTER_TYPE, values: [group.name]}
            //         })
            //     ),
            // filterAppliersOverride:
            //     {[GROUP_COMPARISON_FILTER_TYPE]: this.applySampleIdFilter}
            // }
        );
    }

    @autobind
    private applySampleIdFilter(
        filter: DataFilter<string>,
        mutation: Mutation
    ): boolean {
        const group = this.props.store.groupedSamples.find(
